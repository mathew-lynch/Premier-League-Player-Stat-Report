...
`r renderUI({HTML(paste("<b>", input$player, "</b>"))})` has scored `r renderText({goals()})` goal(s) in the 2021/2022 season from an xG of `r renderText({round(xG(),2)})`, a `r renderText({pos_neg()})` delta of `r renderText({round(xG_delta(),2)})`. This means that he is `r renderText({performance()})` his xG this season.
