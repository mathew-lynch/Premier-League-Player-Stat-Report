---
title: "Premier League 2022/2023 Player Stat Report"
output: 
  html_document:
    theme: united
    highlight: tango
...
`r renderUI({HTML(paste("<b>", team(), "</b>"))})` have won `r renderText({wins()})` matches in the 2022/2023 Premier League season, drawing `r renderText({draws()})` and losing `r renderText({losses()})`.
...
`r renderUI({HTML(paste("<b>", input$player, "</b>"))})` has scored `r renderText({goals()})` goal(s) in the 2022/2023 season from an xG of `r renderText({round(xG(),2)})`, a `r renderText({pos_neg()})` delta of `r renderText({round(xG_delta(),2)})`. This means that he is `r renderText({performance()})` his xG this season.
...
# Filter by 2022/2023 season and by goals
season_goals <- reactive({(
  if (nrow(shots_data()) == 0) {
    b = data.frame()
  } else {
    b = shots_data()[shots_data()$year == "2022" & shots_data()$result == "Goal",]
  })
  return(b)
})
