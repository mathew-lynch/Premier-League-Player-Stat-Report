---
title: "Premier League 2020/2021 Player Stat Report"
output: 
  html_document:
    theme: united
    highlight: tango
...
player_xG <- player_xG %>%
  mutate(Performance = case_when(
    goals - xG > 0 ~ "overperformed",
    goals - xG < 0 ~ "underperformed",
    goals - xG == 0 ~ "matched"
  ))
...
player_xG <- player_xG %>%
  mutate(Performance_A = case_when(
    assists - xA > 0 ~ "overperformed",
    assists - xA < 0 ~ "underperformed",
    assists - xA == 0 ~ "matched"
  ))
...
`r renderUI({HTML(paste("<b>", team(), "</b>"))})` won `r renderText({wins()})` of their 38 matches in the 2020/2021 Premier League season, drawing `r renderText({draws()})` and losing the remaining `r renderText({losses()})`.

Their home win percentage of `r renderText({label_percent()(home_win())})` ranked `r renderText({home_win_ranking()})` in the league, and their away win percentage of `r renderText({label_percent()(away_win())})` ranked at `r renderText({away_win_ranking()})`.

`r renderUI({HTML(paste("<b>", input$player, "</b>"))})` scored `r renderText({goals()})` goals in the 2020/2021 season from an xG of `r renderText({round(xG(),2)})`, a `r renderText({pos_neg()})` delta of `r renderText({round(xG_delta(),2)})`. This means that he `r renderText({performance()})` his xG this season.

The `r renderUI({HTML(paste("<b>", team(), "</b>"))})` player also had an xA of `r renderText({round(xA(),2)})`, from which he recorded `r renderText({assists()})` assists, a `r renderText({pos_neg_A()})` delta of `r renderText({round(xA_delta(),2)})`. He `r renderText({performance_A()})` his xA this season.
...
# Filter by 2020/2021 season and by goals
season_goals <- reactive({(
  if (nrow(shots_data()) == 0) {
    b = data.frame()
  } else {
    b = shots_data()[shots_data()$year == "2020" & shots_data()$result == "Goal",]
  })
  return(b)
})
